      ...
      switch (type.toUpperCase()) {
        case "TEMP":
          this.pickedData = MeteoDataLoader.data[`climatic${year}`].filter(
            row =>
              row.Region === region.toUpperCase() && row.Miesiąc >= from && row.Miesiąc <= to
          );
          result = this.pickedData.reduce((a, b) => a + b.Średnia_miesięczna_temperatura, 0) / this.pickedData.length
          break;
        case "HUMIDITY":
          this.pickedData = MeteoDataLoader.data[`climatic${year}`].filter(
            row =>
          ...
          result = this.pickedData.reduce((a, b) => a + b.Średnia_miesięczna_wilgotność_względna, 0) / this.pickedData.length
          break;
        default:
          ...
          result = this.pickedData.reduce((a, b) => a + b.Miesięczna_suma_opadów, 0) / this.pickedData.length
          break;
      }
    }
