...
class YieldsDataLoader {
  ...
  static single(year, yieldName, columnName, region) {
    ...
    if (
      typeof this.data[yieldName] !== "undefined" &&
      typeof this.data[yieldName][year] !== "undefined"
    ) {
      return this.data[yieldName][year][regionRow][columnName]
        .replace(/\s/g, "")
        .replace(/,/g, ".");
    }
    return null;
  ...
  static async loadSingleFile(year, yieldName) {
    let parsedData = [];
    await d3.csv("data/yields/" + year + "_" + yieldName + ".csv", data =>
  ...
  static async loadData() {
    await this.asyncForEach(this.yieldsTypes, async y => {
      let year = 2003;
      for (year; year <= 2016; year++) {
        await this.loadSingleFile(year, y);
      }
    });
    return this.data;
  }

//  Need to create async forEach
